  ...
  $.popup = function () {
    ...
    /*  -------------------------------------------------------------------------------------------
      ...
      me.confirm = function () {
        ...
        if (isstr(param[1])) {
          _title = param[1]
          ;(_onConfirm = setfn(param[2])), (_onCancel = setfn(param[3]))
        }
    ...
    var build = function () {
      ...
      outer
        .append(content.append(title).append(message))
        .append(toolbar.addClass(me.settings.buttonDisplay))

      // If position is specified
      if (notnull(me.settings.position))
        container.addClass(me.settings.position)
      ...
      if (true == me.settings.closeOverlay)
      ...
      if (notnull(me.settings.opacity)) {
        ...
        if ('msie8' == agent)
          overlay.addClass('opacity-' + me.settings.opacity.toFixed(1) * 100)
      }

      // If overlay is disabled
      if (false == me.settings.overlay) {
        overlay.remove()
        if ('msie9' == agent || 'msie8' == agent)
          container.addClass('ie-overlay-false')
      }
      ...
      if (true == me.settings.closeEsc) {
        $(document).keydown(function (e) {
          ...
          if (e.keyCode == 27) {
            ...
            if (
              true == instance.settings.closeEsc &&
      ...
      /*  -----------------------------------------------------------------------------------------------------
                ...
                -----------------------------------------------------------------------------------------------------
            */ if (
        me.settings.buttons !== false
      )
        $.each(me.settings.buttons, function (btnkey, btnvalue) {
          var item = {},
            btn = button.clone()
          ...
          btn
            .on('click', function (e) {
              ...
              if (fn(item.action)) {
                var close = null
                if (me.settings.dialog == 'alert')
                  close = item.action.call(me, $container, btn)
                else if (me.settings.dialog == 'confirm')
                  close = item.action.call(me, $container, btn)
                else if (me.settings.dialog == 'prompt')
                  close = item.action.call(me, $container, $prompt, btn)
                else if (me.settings.dialog == 'form')
                  close = item.action.call(me, $container, $form, btn)
                if (false !== close) me.close()
    ...
    me.show = function () {
      ...
      if (animated) {
        ...
        $container.addClass('shown').fadeIn(function () {
    ...
    me.height = function (value, callback) {
      if (typeof value === 'undefined')
        return $content.outerHeight() + $toolbar.outerHeight()
    ...
    me.position = function (position) {
      ...
      $container.removeClass(me.settings.position).addClass(position)
    ...
    me.onConfirm = function (func) {
      ...
      if (undef(func)) {
        if (fn(me.settings.onConfirm)) {
          ...
          if ('prompt' == dialog)
            me.settings.onConfirm.call(me, $container, $prompt)
        }
        return me
      }
    ...
    me.onSubmit = function (func) {
      if (undef(func)) {
        if (fn(me.settings.onSubmit))
          me.settings.onSubmit.call(me, $container, $form)
        return me
      }
    ...
    me.onCancel = function (func) {
      ...
      if (undef(func)) {
        if (fn(me.settings.onCancel)) {
          ...
          if ('prompt' == dialog)
            me.settings.onCancel.call(me, $container, $prompt)
