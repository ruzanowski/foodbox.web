...
import {
    ...
    AdditionalsType,
    CaloriesDto,
    CaloriesServiceProxy, CreateOrderBasketItemDto,
    DiscountDto,
...
export class ItemsService implements OnInit {
    ...
    getPriceNominal(item: CreateOrderBasketItemDto) {
        return (this.getProduct(item.productId).priceNet
            ...
            * item.deliveryTimes.length
            + (item.cutleryFeeId == 0 ? 1 : (this.getAdditionalCutlery(item.cutleryFeeId)?.valueGross || 0) * item.deliveryTimes.length * item.count);
    }
    ...
    getPriceNominalByDialog(item: FoodMenuDialog) {
        return (this.getProduct(item.productId).priceNet
            ...
            * item.periodLengthInDays
            + (!item.cutleryIncluded? 1 : (this.getAdditionalCutlery(undefined)?.valueGross || 0) * item.periodLengthInDays * item.count);
