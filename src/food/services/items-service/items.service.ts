...
export class ItemsService implements OnInit{
    ...
    constructor(private caloriesService: CaloriesServiceProxy,
    ...
    getAdditionalDelivery(id: number): AdditionalsDto {
        if(id === undefined){
            return (this.additionals.filter(x => x.type === AdditionalsType._1) || [])[0];

        }

        return (this.additionals.filter(x => x.type === AdditionalsType._1 && id === x.id) || [])[0];
    }

    getAdditionalCutlery(id : number): AdditionalsDto {
        if(id === undefined){
            return (this.additionals.filter(x => x.type === AdditionalsType._0) || [])[0];
        }

        return (this.additionals.filter(x => x.type === AdditionalsType._0 && id === x.id) || [])[0];
    }
