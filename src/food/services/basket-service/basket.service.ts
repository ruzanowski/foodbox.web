...
export class BasketService implements OnChanges {
    ...
    remove(item: CreateOrderBasketItemDto): void {
        ...
        this.reCalculateTotals();
        abp.notify.error(item.count + 'x ' + this.itemsService.getProduct(item.productId).name, 'Usunięto z koszyka');
    }
    ...
    public transformToSimpleBasketItem(
        ...
        return CreateOrderBasketItemDto.fromJS({
            ...
            count: dialog.count,
            cutleryFeeId: this.itemsService.getAdditionalCutlery(undefined)?.id || null,
    ...
    private reCalculateTotals() {
        ...
        this.basket.totalPrice = this.basket.items.reduce((sum, current) => {
            ...
            const priceWithDiscount = price * (1 - (this.itemsService.getDiscountIfAny(this.getTotalDays())?.value || 0));
            priceWithoutDiscountAndFeesTotal += price;
            discountSavesTotal += price - priceWithDiscount;
            const deliveryCostTemp = (this.itemsService.getAdditionalDelivery(undefined)?.valueGross || 0);
            deliveryCostTotal += deliveryCostTemp;

            const cutleryCostTemp = current.cutleryFeeId == 0 ? 0 : (this.itemsService.getAdditionalCutlery(current.cutleryFeeId)?.valueGross || 0) * current.deliveryTimes.length * current.count;
