  ...
  $(password1).on('keyup', function (e) {
    if (VryStrongPass.test(password1.val())) {
      ...
      passwordsInfo
        .removeClass()
        .addClass('strongpass')
        .html('Strong! (Enter special chars to make even stronger')
    } else if (MediumPass.test(password1.val())) {
      passwordsInfo
        .removeClass()
        .addClass('goodpass')
        .html('Good! (Enter uppercase letter to make strong)')
    } else if (WeakPass.test(password1.val())) {
      passwordsInfo
        .removeClass()
        .addClass('stillweakpass')
        .html('Still Weak! (Enter digits to make good password)')
    } else {
      passwordsInfo
        .removeClass()
        .addClass('weakpass')
        .html('Very Weak! (Must be 5 or more chars)')
    }
  })
